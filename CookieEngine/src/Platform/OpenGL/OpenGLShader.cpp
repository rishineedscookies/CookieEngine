...
namespace Cookie {
	...
	OpenGLShader::OpenGLShader(const std::string& vertexSrc, const std::string& fragmentSrc)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			CK_CORE_ASSERT(false, "Vertex shader compilation failure!");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			CK_CORE_ERROR("{0}", infoLog.data());
			CK_CORE_ASSERT(false, "Fragment shader compilation failure!");
			return;
		}
	...
	void OpenGLShader::UploadUniformMat4(const std::string& name, const mathfu::mat4& matrix) const
	{
